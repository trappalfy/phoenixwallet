...
> **Ребрендинг 2026: Phoenix → Perigee.** Композиция, метрика, сетка и поведение из этого
> документа остаются в силе как есть — они не зависели от бренда. Три раздела ниже устарели
> и приведены только как история:
> - **§3.1 Палитра** — огненная (`--ember`, `--ember-hot` и т.д.) заменена на фиолетово-синюю
>   неоновую; актуальные значения — в `src/theme/colors.ts` и `src/index.css`.
> - **§7.5 Настройка характера дыма** — константы `SMOKE`/`EMBER` в `smoke.ts` переименованы
>   в `DUST`/`KNOT` и перекрашены, к дымовому полю добавлено разреженное звёздное поле.
> - **§21 Чего не делать** — пункт про частицы-искры был обоснован тем, что бренд Phoenix уже
>   держит две огненные метафоры (дым с углями + ember-волна). Бренд сменился, но третий
>   эффект по-прежнему не добавлялся — только звёзды внутри уже существующего дымового шейдера.
>
> Слово `PHOENIX` в §13 (wordmark) заменено на `PERIGEE` — семь букв, `--wm-rows: 24` не
> пересчитывался.
